    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          {currentPage === 'project-detail' && <ProjectDetail selectedProject={selectedProject} updateProject={updateProject} addMilestone={addMilestone} updateMilestone={updateMilestone} deleteMilestone={deleteMilestone} calculateProgress={calculateProgress} />}
        </main>
      </div>
      ...
      <form onSubmit={submit} className="space-y-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border space-y-6">
          ...
          <div>
            ...
            <input type="number" value={form.totalCost} onChange={(e) => setForm({ ...form, totalCost: e.target.value })} placeholder="Amount in USD" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border space-y-6">
          <h2 className="text-xl font-bold">Add Milestones</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium mb-2">Milestone Name</label>
              <input type="text" value={current.name} onChange={(e) => setCurrent({ ...current, name: e.target.value })} placeholder="Milestone name" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Target Date</label>
              <input type="date" value={current.targetDate} onChange={(e) => setCurrent({ ...current, targetDate: e.target.value })} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium mb-2">Description</label>
            <textarea value={current.description} onChange={(e) => setCurrent({ ...current, description: e.target.value })} placeholder="Milestone description" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" rows="2"></textarea>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium mb-2">Budget</label>
              <input type="number" value={current.budget} onChange={(e) => setCurrent({ ...current, budget: e.target.value })} placeholder="Amount" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Progress (%)</label>
              <input type="range" min="0" max="100" value={current.progress} onChange={(e) => setCurrent({ ...current, progress: parseInt(e.target.value) })} className="w-full" />
              <span className="text-sm text-slate-600">{current.progress}%</span>
            </div>
          </div>
          <button type="button" onClick={addMilestoneToList} className="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Add Milestone</button>
          {milestones.length > 0 && (
            <div className="border-t pt-4">
              <h3 className="font-semibold mb-2">Added Milestones</h3>
              <div className="space-y-2">
                {milestones.map((m, idx) => (
                  <div key={idx} className="flex justify-between items-center bg-blue-50 p-2 rounded">
                    <span className="text-sm">{m.name} - {m.targetDate}</span>
                    <button type="button" onClick={() => setMilestones(milestones.filter((_, i) => i !== idx))} className="text-red-600 text-sm">Remove</button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <button type="submit" className="w-full px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700">Create Project</button>
      </form>
    </div>
  );
};

const ProjectDetail = ({ selectedProject, updateProject, addMilestone, updateMilestone, deleteMilestone, calculateProgress }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setEditing(!editing)} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-lg font-semibold">{selectedProject.client}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-slate-600">Assigned To</label>
            <p className="text-lg font-semibold">{selectedProject.assignedTo}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-lg">{selectedProject.startDate}</p>
          </div>
        </div>
        <div className="border-t pt-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="text-sm font-medium text-slate-600">Total Budget</label>
              <p className="text-2xl font-bold text-green-600">${(selectedProject.totalCost / 1000000).toFixed(1)}M</p>
            </div>
            <div>
              <label className="text-sm font-medium text-slate-600">Spent</label>
              <p className="text-2xl font-bold text-purple-600">${((selectedProject.actualSpent || 0) / 1000000).toFixed(1)}M</p>
            </div>
            <div>
          ...
          {(selectedProject.milestones || []).map(m => (
            <div key={m.id} className="border rounded-lg p-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <button onClick={() => deleteMilestone(selectedProject.id, m.id)} className="text-red-600 hover:text-red-800">
                  <Trash2 className="w-5 h-5" />
                </button>
              </div>
              <div className="flex justify-between items-center mb-2 text-sm">
                <span>Target: {m.targetDate}</span>
                <span>Budget: ${(m.budget / 1000).toFixed(0)}K</span>
              </div>
              <div>
...
root.render(<ArchProjectManagement />);
